...
pub async fn updater() -> Result<()> {
    ...
    let latest_release: &Release = releases.first().ok_or_else(|| {
        eyre!("The repository {GITHUB_REPO_OWNER}/{GITHUB_REPO_NAME} has no releases.")
    })?;
    // Get bad versions
    let bad_versions: BadVersions = BadVersions::fetch(latest_release, Some(&releases)).await?;
