    ...
    for MenuElementList<'_, ItemId, Item>
{
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let scroll_offset: usize = if let Some(selected_item_id) = self.selected() {
            self.items
                .iter()
                .position(|(item_id, _): &(ItemId, Item)| -> bool { *item_id == selected_item_id })
                .unwrap()
        } else {
            0
        };
        ...
        let (items_area, selected_note_area): (Rect, Option<Rect>) = if self.counter {
        ...
        for (current_item_id, current_item) in self.items.iter().skip(scroll_offset) {
