...
impl EntityTemplates {
    ...
    pub async fn process_dir(
        ...
        let path: &Path = path_to_process.map_or(original_path, |path| path);
        let mut entity_templates: Self = Self::default();
        for entry in fs::read_dir(path)? {
            let dir_entry = entry?;
            let entry_path = dir_entry.path();
            if path == original_path
                && path.file_name() == Some("entity_templates".as_ref())
                && dir_entry.file_name() == "core"
                && entry_path.is_dir()
            {
                continue;
            }
            ...
            if entry_path.is_dir() {
                entity_templates.merge(
                    Self::process_dir(original_path, Some(&entry_path), duplicates_possible)
                        .await?,
                    &merge_priority,
                )?;
                ...
                entity_templates.merge(
                    Self::load(&entry_path).await.wrap_err_with(|| {
                        format!(
        ...
        match *priority {
            MergePriority::Self_ => {
                for (key, value) in other.0 {
                    let _: &mut Entity = self.0.entry(key).or_insert(value); // The value in other has no use
                }
            }
            MergePriority::Other => {
                for (key, value) in other.0 {
                    let _: Option<Entity> = self.0.insert(key, value);
                }
            }
