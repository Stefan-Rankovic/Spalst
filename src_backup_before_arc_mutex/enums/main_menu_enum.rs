...
use crate::enums::ManagePlaythroughsMenu;
use std::collections::HashMap;
use strum::IntoEnumIterator;
use strum_macros::{EnumDiscriminants, EnumIter};
use tokio::time::Instant;

#[deprecated = "Use MenuElementsMainMenuSelected instead."]
...
pub enum MainMenuEnum {
    ...
    ManagePlaythroughs(ManagePlaythroughsMenu<'static, 'static>), // todo: these statics are
    // nonsense as the type is
    // deprecated anyway and isn't
    // used
    Achievements,
    Settings,
    Quit,
    ...
    fn from(discriminant: MainMenuEnumDiscriminants) -> Self {
        match discriminant {
            ...
            MainMenuEnumDiscriminants::ManagePlaythroughs => {
                Self::ManagePlaythroughs(ManagePlaythroughsMenu::build_select(HashMap::new())) // todo: the new hashmap is absolutely horrendous here; but because the enum is deprecated, it's fine (just remove this)
            }
